/* ============================================================
   FAQ Hub — Con pestañas FAQs / Guías
   ============================================================ */
...
function norm(str){ return String(str).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,''); }
...
async function loadData(){
  try {
    ...
    $('loading').innerHTML = '⚠️ Error al cargar los datos.<br><small>' + esc(err.message) + '</small>';
    console.error(err);
  }
}
...
function currentItems(){ return state.tab === 'faqs' ? DATA.faqs : DATA.guides; }
...
function buildFilters(){
  ...
  // fuentes con contenido en la pestaña activa
  ...
  DATA.categories.filter(c => catsInTab.includes(c.id)).forEach(c => {
    ...
    b.onclick = () => { state.cat = (state.cat === c.id ? 'all' : c.id); render(); closeDrawer(); };
...
function render(){
  ...
  // mostrar/ocultar acciones de FAQ
    ...
    <div class="guide-top">
      ...
      <div class="guide-body">
        ...
        <div class="guide-meta">
          ...
          <span class="src-tag">${esc(src.name)}</span>
          <a class="guide-link" href="${esc(g.url || g.url_original || '#')}" target="_blank" rel="noopener">Descargar PDF</a>
        </div>
      </div>
    </div>
  </article>`;
...
document.querySelectorAll('.tab-btn').forEach(b => {
  b.onclick = () => {
    ...
    state.src = 'all'; state.cat = 'all';
    $('searchInput').value = ''; state.query = ''; $('clearSearch').classList.remove('show');
    buildFilters();
    render();
  };
});
